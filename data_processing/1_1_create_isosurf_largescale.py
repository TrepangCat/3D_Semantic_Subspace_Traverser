...
def get_normalize_mesh(model_file, norm_mesh_sub_dir, x_scale, y_scale, z_scale):
    # total = 16384
...
def create_sdf_obj(sdfcommand, marching_cube_command, cat_mesh_dir, cat_norm_mesh_dir, cat_sdf_dir, obj,
    ...
    scale_list = [[0.75, 1.0, 1.0], [1.0, 0.75, 1.0], [1.0, 1.0, 0.75],
                  # [1.25, 1.0, 1.0], [1.0, 1.25, 1.0], [1.0, 1.0, 1.25],
                  ]

    for x_scale, y_scale, z_scale in scale_list:

        sdf_sub_dir = os.path.join(cat_sdf_dir, obj + f'_x{x_scale}_y{y_scale}_z{z_scale}')
        if not os.path.exists(sdf_sub_dir): os.makedirs(sdf_sub_dir)

        norm_mesh_sub_dir = os.path.join(cat_norm_mesh_dir, obj + f'_x{x_scale}_y{y_scale}_z{z_scale}')
        ...
        if not os.path.exists(os.path.join(norm_mesh_sub_dir, 'pc_norm.obj')):
            norm_obj_file = get_normalize_mesh(model_file, norm_mesh_sub_dir, x_scale, y_scale, z_scale)
        norm_obj_file = os.path.join(norm_mesh_sub_dir, 'pc_norm.obj')
...
if __name__ == "__main__":
    ...
    data = {
        ...
        "raw_dirs": {
            ...
            "norm_mesh_dir": "/home/brl/dataDisk2/wrw/dataset/ShapeNetDataSet_subset/ShapeNetCore.v1_processed_largeScale",
            "sdf_dir": "/home/brl/dataDisk2/wrw/dataset/ShapeNetDataSet_subset/ShapeNetCore.v1_processed_largeScale",
        }
    }
