...
__all__ = ['postprocess', 'factorize_weight',
           'HtmlPageVisualizer']
...
def factorize_weight(generator, gan_type, layer_idx='all'):
    ...
    block_resolutions = [4] + [2 ** (i // 2) for i in range(6, int(np.log2(img_resolution) + 1) * 2)] + [img_resolution]
    for idx in layers:
        layer_name = f'b{block_resolutions[idx]}'
        if gan_type == 'stylegan2' and idx == 0:
            weight = generator.synthesis.__getattr__(layer_name).conv1.affine.weight.T
        # if gan_type == 'pggan':
        #     weight = generator.__getattr__(layer_name).weight
        #     weight = weight.flip(2, 3).permute(1, 0, 2, 3).flatten(1)
        elif gan_type in ['stylegan', 'stylegan2'] and idx < num_layers - 1:
            if idx % 2 == 1:
                weight = generator.synthesis.__getattr__(layer_name).conv0.affine.weight.T
            else:
                weight = generator.synthesis.__getattr__(layer_name).conv1.affine.weight.T
        elif gan_type in ['stylegan', 'stylegan2'] and idx == num_layers - 1:
            weight = generator.synthesis.__getattr__(
                layer_name).torgb.affine.weight.T  # 这是专门为 skip 模式的stylegan2-adas设计的
        weights.append(weight.cpu().detach().numpy())
    weight = np.concatenate(weights, axis=1).astype(np.float32)
    weight = weight / np.linalg.norm(weight, axis=0, keepdims=True)  # 除以其二范数
